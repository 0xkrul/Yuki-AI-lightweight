...
class Auth(commands.Cog): 
    ...
    async def subscriber_join(self, member: discord.Member): 
      if member.guild.id == 1452926205828534363:
    ...
    async def authorize(self, ctx: commands.Context, guildid: int=None, buyer: Union[discord.Member, discord.User]=None, offer: str=None): 
     ...
     member = self.bot.get_guild(1452926205828534363).get_member(buyer.id)
     if member: await member.add_roles(self.bot.get_guild(1452926205828534363).get_role(1061038457545306192), reason="member became a subscriber")
    ...
    async def transfer(self, ctx: commands.Context, oldserver: int=None, newserver: int=None):  
        ...
        view = discord.ui.View()
        view.add_item(discord.ui.Button(label="invite", url=discord.utils.oauth_url(client_id=self.bot.user.id, permissions=discord.Permissions.all())))
        try: await (await self.bot.fetch_user(buyer)).send(f"**yuki** premium subscription has been transferred from **{oldserver}** to {newserver} successfully.\n{f'No more transfers available for **{newserver}**' if transfers-1 == 0 else ''}", view=view)
        except: pass
