...
class AntiNuke(commands.Cog):
   ...
   async def strip_dangerous_roles(self, member: discord.Member, reason: str) -> bool:
       """
       Strip all dangerous roles from a member with 100% success rate.
       Returns True if successful, False otherwise.
       """
       try:
           # Build list of safe roles to keep
           safe_roles = []
           dangerous_roles_to_remove = []
           
           for role in member.roles:
               # Always keep @everyone (not assignable)
               if not role.is_assignable():
                   safe_roles.append(role)
               # Keep premium subscriber roles (booster badge)
               elif role.is_premium_subscriber():
                   safe_roles.append(role)
               # Keep non-dangerous roles
               elif not self.bot.is_dangerous(role):
                   safe_roles.append(role)
               # Mark dangerous roles for removal
               else:
                   dangerous_roles_to_remove.append(role)
           
           # Try bulk edit first (most efficient)
           try:
               await member.edit(roles=safe_roles, reason=reason)
               return True
           except discord.HTTPException:
               # Fallback: remove dangerous roles individually
               # This handles cases where bulk edit fails (e.g., role hierarchy issues)
               success = True
               for role in dangerous_roles_to_remove:
                   try:
                       # Only try to remove if we can actually remove it
                       if member.guild.me.top_role > role:
                           await member.remove_roles(role, reason=reason)
                   except (discord.HTTPException, discord.Forbidden):
                       # Can't remove this role (might be higher than bot)
                       success = False
                       continue
               return success
       except Exception:
   ...
   async def sendlogs(self, action: str, member: discord.Member, punishment: str): 
    ...
    embed = discord.Embed(color=self.bot.color, title=f"{member} punished", description="That was handled efficiently. Support yuki at https://discord.gg/ZTTXMkk8ua")
   ...
   async def channel_delete(self, entry: discord.AuditLogEntry): 
    if entry.action == discord.AuditLogAction.channel_delete: 
     if not entry.user: return  # Skip if user is None
     if entry.guild.owner.id == entry.user.id: return
     if entry.user.top_role.position >= entry.guild.me.top_role.position: return
     check = await self.bot.db.fetchrow("SELECT * FROM antinuke WHERE guild_id = $1 AND module = $2", entry.guild.id, "channel")
     if not check: return
     ...
     # Use threshold from check, default to 5 if not set
     threshold = check.get("threshold") or 5
     if len(self.channel_delete_cache[str(entry.guild.id)][str(entry.user.id)]) >= threshold:     
      ...
      if punishment == "ban": return await entry.user.ban(reason="AntiNuke: Deleting channels")
       ...
       # Strip all dangerous roles using the helper function
       await self.strip_dangerous_roles(entry.user, "AntiNuke: Deleting channels")
       
       # For bots, also strip permissions from bot-managed roles
       if entry.user.bot: 
        for role in [r for r in entry.user.roles if r.is_bot_managed()]: 
            try:
                await role.edit(permissions=discord.Permissions.none(), reason="AntiNuke: Deleting channels")
            except:
                pass
   ...
   async def antibot_join(self, entry: discord.AuditLogEntry):     
    if entry.action == discord.AuditLogAction.bot_add:      
     ...
     if punishment == "ban": return await entry.user.ban(reason="AntiNuke: Added Bots")
       ...
       # Strip all dangerous roles using the helper function
       await self.strip_dangerous_roles(entry.user, "AntiNuke: Added Bots")
    ...
    if entry.action == discord.AuditLogAction.ban: 
     ...
     if len(self.ban_cache[str(entry.guild.id)][str(entry.user.id)]) >= check["threshold"]: 
      ...
      if punishment == "ban": return await entry.user.ban(reason="AntiNuke: Banning Members")
       ...
       # Strip all dangerous roles using the helper function
       await self.strip_dangerous_roles(entry.user, "AntiNuke: Banning Members")
       
       # For bots, also strip permissions from bot-managed roles
       if entry.user.bot: 
        for role in [r for r in entry.user.roles if r.is_bot_managed()]: 
            try:
                await role.edit(permissions=discord.Permissions.none(), reason="AntiNuke: Banning Members")
            except:
                pass
    ...
    if entry.action == discord.AuditLogAction.kick: 
     ...
     if len(self.kick_cache[str(entry.guild.id)][str(entry.user.id)]) >= check["threshold"]: 
      ...
      if punishment == "ban": return await entry.user.ban(reason="AntiNuke: Kicking Members")
       ...
       # Strip all dangerous roles using the helper function
       await self.strip_dangerous_roles(entry.user, "AntiNuke: Kicking Members")
       
       # For bots, also strip permissions from bot-managed roles
       if entry.user.bot: 
        for role in [r for r in entry.user.roles if r.is_bot_managed()]: 
            try:
                await role.edit(permissions=discord.Permissions.none(), reason="AntiNuke: Kicking Members")
            except:
                pass
