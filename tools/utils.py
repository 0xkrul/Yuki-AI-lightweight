...
class StartUp:
 ...
 async def loadcogs(self): 
  # Skip these files to avoid conflicts
  skip_cogs = ["moderation_ai_optimized", "moderation"]  # moderation_ai is already loaded, moderation has duplicate commands
  ...
  for fil in os.listdir("./cogs"):
   if fil.endswith(".py"):
    ...
    if cog_name in skip_cogs:
     print(f"Skipped plugin {cog_name} (avoided conflict)".lower())
     continue
    try:
     await self.load_extension(f"cogs.{cog_name}")
     print(f"Loaded plugin {cog_name}".lower())
    except Exception as e: print("failed to load %s %s".lower(), cog_name, e)
...
async def create_db(self: commands.AutoShardedBot): 
  ...
  await self.db.execute("CREATE TABLE IF NOT EXISTS music_dj (guild_id BIGINT, channel_id BIGINT, dj_id BIGINT)")
  await self.db.execute("CREATE TABLE IF NOT EXISTS music_mixers (guild_id BIGINT, channel_id BIGINT, mixer_id BIGINT)")
  ...
  await self.db.execute('CREATE TABLE IF NOT EXISTS reskin (user_id BIGINT PRIMARY KEY, avatar_url TEXT NOT NULL,name TEXT NOT NULL)')
  await self.db.execute('CREATE TABLE IF NOT EXISTS vanity_settings (guild_id BIGINT PRIMARY KEY, vanity_phrase TEXT, role_id BIGINT)')
  await self.db.execute('CREATE TABLE IF NOT EXISTS only_cmd (guild_id BIGINT, channel_id BIGINT)')
  await self.db.execute('CREATE TABLE IF NOT EXISTS restrict_role (guild_id BIGINT, role_id BIGINT, channel_id BIGINT)')
  await self.db.execute('CREATE TABLE IF NOT EXISTS disabled_commands (guild_id BIGINT, command TEXT)')
