...
class Client(object): 
  ...
  async def send_success(self, ctx: Union[commands.Context, discord.Interaction], message: str, ephemeral: bool=True) -> discord.Message or None: 
   # Success with green color and checkmark
   if isinstance(ctx, commands.Context): return await ctx.reply(embed=discord.Embed(color=0x3ba55d, description=f"{self.bot.yes} {ctx.author.mention}: {message}"))
   else: return await ctx.response.send_message(embed=discord.Embed(color=0x3ba55d, description=f"{self.bot.yes} {ctx.user.mention}: {message}"), ephemeral=ephemeral)
  ...
  async def send_error(self, ctx: Union[commands.Context, discord.Interaction], message: str, ephemeral: bool=True) -> discord.Message or None: 
   # Error with red color and X mark
   if isinstance(ctx, commands.Context): return await ctx.reply(embed=discord.Embed(color=0xed4245, description=f"{self.bot.no} {ctx.author.mention}: {message}"))
   else: return await ctx.response.send_message(embed=discord.Embed(color=0xed4245, description=f"{self.bot.no} {ctx.user.mention}: {message}"), ephemeral=ephemeral)
  ...
  async def send_warning(self, ctx: Union[commands.Context, discord.Interaction], message: str, ephemeral: bool=True) -> discord.Message or None: 
   # Warning with orange/yellow color and X mark
   if isinstance(ctx, commands.Context): return await ctx.reply(embed=discord.Embed(color=0xfaa81a, description=f"{self.bot.warning} {ctx.author.mention}: {message}"))
   else: return await ctx.response.send_message(embed=discord.Embed(color=0xfaa81a, description=f"{self.bot.warning} {ctx.user.mention}: {message}"), ephemeral=ephemeral)
